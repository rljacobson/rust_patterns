...
Rust's language features enabling information hiding and encapsulation are much simpler
and fewer in number than those in C++. In Rust, items and fields are private by default,
and the visibility modifier `pub` is intentionally coarse-grained, limited to `pub(crate)`,
`pub(super)`, and `pub(in path)`. There is no mechanism to grant selective access to
private items. Rust has no concept of inheritance. Rust uses traits, which define
a set of method signatures, to define shared interfaces. A trait can have one or more
*trait constraints*, which look like superclasses if you squint, but which are really
