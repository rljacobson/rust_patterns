...
The fundamental requirement for soundness is that a mutable borrow of the internal `Vec` (or
its owned memory) can never occur simultaneously with any other borrow. The only mutable borrows
that exist are the temporary borrows in the method implementations we control. Therefore,
`ValueVec` methods must not be re-entered while another method is still executing on the same
instance. Doing so would create overlapping borrows of the internal `Vec` and cause undefined
behavior. Our implementation makes no such explicit calls, so the only way this can happen
is _implicitly_, in particular when the compiler inserts a call to the `V` type's `Drop`
implementation. You would have to try pretty hard to trigger this, but it *is* possible.

There are a few ways to deal with this requirement:

- Require `V` to be `Copy`. `Copy` types are not allowed to have destructors. This is the simplest and easiest solution,
  but it limits `ValueVec<V>` to a far more restrictive category of types than is necessary.
- It _might_ be possible to carefully restrict the API of `ValueVec` and the implementation of its methods so that
  re-entrancy does not cause UB, but this is very delicate and an open question.
- Enforce a documented contract that `V` must not require destruction by wrapping all `V` values in
  `std::mem::ManuallyDrop<V>`, which will prevent any destructor from running.
- Prevent creation of a `ValueVec<V>` with an assertion of the form
  `assert!(!needs_drop::<T>(), "Type must not have a destructor")`. (Perhaps used in combination with the strategy of
  the previous bullet.)
- Just make users pinky promise not to do weird things in the `Drop` implementation of `V`. This dispenses with
...
/// Cloning / copying getters exist if `V` implements `Clone`/`Copy`. See the module-level docs for important notes
/// about soundness.
...
impl<V> ValueVec<V> {
    ...
    /// Returns a **clone** of the element at `index`. Panics if `index` is out of bounds.
    ///
    /// This is a cloning version of [`ValueVec::at`].
    ...
    /// Returns a **copy** of the element at `index` Panics if `index` is out of bounds.
    ///
    /// Use [`ValueVec::get_cloned`] for a cloning version of this method.
    ...
    ///
    /// Use [`into`](ValueVec::into) or [`Vec::from`] to consume the `ValueVec`
    /// and perform a zero-cost conversion without cloning.
